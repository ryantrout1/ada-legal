 ...
 *         PUBLIC reviewer queue, newest-unreviewed first.
 *
 * PUBLIC — no auth. This is the no-login feedback surface for Peter, Gina,
 * and Ryan. Scoped to is_test photos only (same as the admin queue), so
 * real claimants' analyses never appear here. Read-only; review submission
 * is handled by POST /api/photo-review/[id].
 *
 * Filters mirror the admin queue: review_state, risk, model_version.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.error('GET /api/photo-review failed', err);
