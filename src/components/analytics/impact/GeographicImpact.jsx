    ...
    <div style={{ backgroundColor: 'white', border: '1px solid var(--slate-200)', borderRadius: '12px', padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
        <div>
          ...
          <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.8125rem', color: 'var(--slate-500)', margin: '2px 0 0' }}>Geographic Distribution</p>
        </div>
      ...
      {showTable ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            <h4 style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.875rem', fontWeight: 700, color: 'var(--slate-800)', margin: '0 0 8px' }}>Top 20 Cities</h4>
            <div style={{ overflow: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr>
                  <th style={{ textAlign: 'left', padding: '6px 8px', fontFamily: 'Manrope, sans-serif', fontSize: '0.6875rem', fontWeight: 700, color: 'var(--slate-500)', borderBottom: '2px solid var(--slate-200)', textTransform: 'uppercase' }}>City</th>
                  <th style={{ textAlign: 'center', padding: '6px 8px', fontFamily: 'Manrope, sans-serif', fontSize: '0.6875rem', fontWeight: 700, color: 'var(--slate-500)', borderBottom: '2px solid var(--slate-200)', textTransform: 'uppercase' }}>Reports</th>
                </tr></thead>
                <tbody>{cityData.map(c => (
                  <tr key={c.name}><td style={{ padding: '6px 8px', fontFamily: 'Manrope, sans-serif', fontSize: '0.8125rem', borderBottom: '1px solid var(--slate-100)' }}>{c.name}</td>
                  <td style={{ padding: '6px 8px', textAlign: 'center', fontWeight: 700, fontFamily: 'Manrope, sans-serif', fontSize: '0.8125rem', borderBottom: '1px solid var(--slate-100)' }}>{c.count}</td></tr>
                ))}</tbody>
              </table>
            </div>
          </div>
          <div>
            <h4 style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.875rem', fontWeight: 700, color: 'var(--slate-800)', margin: '0 0 8px' }}>States by Total Reports</h4>
            <div style={{ overflow: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '500px' }}>
                <thead><tr>
                  {['State', 'Total', 'Businesses', 'Top Violation', 'Attorney Coverage'].map(h => (
                    <th key={h} style={{ textAlign: h === 'State' || h === 'Top Violation' || h === 'Attorney Coverage' ? 'left' : 'center', padding: '6px 8px', fontFamily: 'Manrope, sans-serif', fontSize: '0.6875rem', fontWeight: 700, color: 'var(--slate-500)', borderBottom: '2px solid var(--slate-200)', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>{stateData.map(s => (
                  <tr key={s.state}>
                    <td style={{ padding: '6px 8px', fontFamily: 'Manrope, sans-serif', fontSize: '0.8125rem', fontWeight: 600, borderBottom: '1px solid var(--slate-100)' }}>{s.state}</td>
                    <td style={{ padding: '6px 8px', textAlign: 'center', fontWeight: 700, fontFamily: 'Manrope, sans-serif', fontSize: '0.8125rem', borderBottom: '1px solid var(--slate-100)' }}>{s.total}</td>
                    <td style={{ padding: '6px 8px', textAlign: 'center', fontFamily: 'Manrope, sans-serif', fontSize: '0.8125rem', borderBottom: '1px solid var(--slate-100)' }}>{s.businesses}</td>
                    <td style={{ padding: '6px 8px', fontFamily: 'Manrope, sans-serif', fontSize: '0.75rem', borderBottom: '1px solid var(--slate-100)', textTransform: 'capitalize' }}>{s.topViolationType}</td>
                    <td style={{ padding: '6px 8px', fontFamily: 'Manrope, sans-serif', fontSize: '0.75rem', fontWeight: 600, borderBottom: '1px solid var(--slate-100)', color: s.hasCoverage ? '#16A34A' : '#DC2626' }}>
