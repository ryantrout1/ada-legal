      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            {isPhysical
              ? <Building2 size={20} aria-hidden="true" style={{ color: 'var(--accent)' }} />
              : <Globe size={20} aria-hidden="true" style={{ color: 'var(--link)' }} />
            }
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{
              ...
              color: 'var(--accent-success)', backgroundColor: '#DCFCE7',
              textTransform: 'uppercase', letterSpacing: '0.04em'
            }}>Available</span>
            <SourceBadge source={c.intake_source} />
            {freshness.type === 'new' ? (
              <span style={{ padding: '2px 7px', borderRadius: '4px', fontSize: '0.625rem', fontWeight: 800, color: 'var(--accent-success)', backgroundColor: '#BBF7D0', textTransform: 'uppercase' }}>NEW</span>
            ) : (
        ...
        <div style={{
          ...
          <span style={{ fontFamily: 'Fraunces, serif', fontSize: '1.25rem', fontWeight: 600, color: 'var(--heading)' }}>
            {c.business_type || '—'}
          </span>
        ...
        <div style={{ padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{
            backgroundColor: 'var(--page-bg-subtle)', borderRadius: '12px', padding: '16px',
            display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px'
          }}>
            {summaryItems.map((item, i) => (
              <div key={i}>
                <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.7rem', fontWeight: 700, color: 'var(--body-secondary)', textTransform: 'uppercase', letterSpacing: '0.04em', margin: '0 0 2px' }}>{item.label}</p>
                <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.95rem', fontWeight: 700, color: 'var(--heading)', margin: 0 }}>{item.value}</p>
              </div>
            ))}
            {/* Assistive Tech pills for digital */}
            {!isPhysical && c.assistive_tech?.length > 0 && (
              <div style={{ gridColumn: '1 / -1' }}>
                <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.7rem', fontWeight: 700, color: 'var(--body-secondary)', textTransform: 'uppercase', letterSpacing: '0.04em', margin: '0 0 6px' }}>Assistive Technologies</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {c.assistive_tech.map((t, i) => (
                    <span key={i} style={{
                      ...
                      color: 'var(--link)', backgroundColor: '#DBEAFE'
                    }}>{t}</span>
                  ))}
                </div>
              </div>
          ...
          <div style={{
            ...
            <FileText size={16} aria-hidden="true" style={{ color: 'var(--accent)', marginBottom: '8px', opacity: 0.6 }} />
            <p style={{
              fontFamily: 'Manrope, sans-serif', fontSize: '1rem', color: 'var(--heading)',
          ...
          {c.photos?.length > 0 && (
            <div>
              <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.7rem', fontWeight: 700, color: 'var(--body-secondary)', textTransform: 'uppercase', letterSpacing: '0.04em', margin: '0 0 8px' }}>Evidence Photos</p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
              <div style={{ display: 'flex', gap: '4px' }}>
                {docScore.criteria.map((cr, i) => (
                  <span key={i} style={{ width: '9px', height: '9px', borderRadius: '50%', backgroundColor: cr.met ? docScore.color : 'var(--body-secondary)' }} />
                ))}
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '4px 16px' }}>
              {docScore.criteria.map((cr, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span style={{ fontSize: '0.85rem', color: cr.met ? 'var(--accent-success)' : 'var(--body-secondary)', flexShrink: 0 }}>{cr.met ? '●' : '○'}</span>
                  <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.85rem', color: cr.met ? 'var(--heading)' : 'var(--body-secondary)' }}>{cr.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <ContactIcon size={16} style={{ color: 'var(--link)', flexShrink: 0 }} />
              <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.9375rem', fontWeight: 700, color: 'var(--link)' }}>
                Contact Preference: {contactPrefLabel}
              </span>
            </div>
            <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.8125rem', color: 'var(--link)', margin: 0, lineHeight: 1.5 }}>
        ...
        <div style={{ padding: '0 24px 20px' }}>
          <button type="button" onClick={() => { trackEvent('attorney_connect_clicked', { case_id: c.id, violation_type: c.violation_type }, 'Marketplace'); onInitiate(c); }} style={{
            ...
            color: 'var(--btn-text)', backgroundColor: 'var(--section-label)', border: 'none',
            borderRadius: '10px', cursor: 'pointer', minHeight: '52px',
            transition: 'background-color 0.15s, transform 0.15s'
          }}
            onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--section-label)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
            onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'var(--section-label)'; e.currentTarget.style.transform = 'none'; }}
          >
          ...
          <button ref={closeRef} type="button" onClick={onClose} style={{
            ...
            color: 'var(--body-secondary)', textAlign: 'center', padding: '6px'
          }}
