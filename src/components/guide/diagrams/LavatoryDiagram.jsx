...
const CALLOUTS = [
  ...
  {
    id: 3, label: 'Knee Clearance', section: '§606.2 / §306', color: '#2563EB', textColor: '#1E3A8A',
    x: 100, y: 195,
  ...
  {
    id: 4, label: 'Toe Clearance', section: '§306.2', color: '#7C3AED', textColor: '#5B21B6',
    x: 300, y: 195,
  ...
  return (
    <div className="ada-diagram-wrap" style={{ margin: '32px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', flexWrap: 'wrap', gap: '8px' }}>
        <h3 style={{ fontFamily: 'Fraunces, serif', fontSize: '1.15rem', fontWeight: 700, color: 'var(--heading)', margin: 0 }}>§606 Lavatories & Sinks</h3>
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          ...
          {['Imperial', 'Metric'].map(u => {
            ...
            return (<button key={u} onClick={() => setMetric(u === 'Metric')} aria-pressed={isA} style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.75rem', fontWeight: isA ? 700 : 500, padding: '4px 10px', borderRadius: '6px', border: '1px solid var(--border)', background: isA ? 'var(--dark-bg)' : 'white', color: isA ? 'white' : 'var(--body)', cursor: 'pointer', minHeight: '28px' }}>{u}</button>);
          })}
        </div>
      </div>
      ...
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
        <svg viewBox="0 0 900 320" role="img" aria-labelledby="lav-title" style={{ width: '100%', height: 'auto', display: 'block' }}>
          ...
          <text x="200" y="200" textAnchor="middle" fontFamily="Manrope, sans-serif" fontSize="6.5" fill="#9D174D" fontWeight="600">PIPE COVERS</text>

          {/* Knee clearance zone */}
          <rect x="108" y="108" width="184" height="134" rx="2" fill="#2563EB" opacity="0.04" stroke="#2563EB" strokeWidth="1" strokeDasharray="5 3" />
          <text x="200" y="165" textAnchor="middle" fontFamily="Manrope, sans-serif" fontSize="7" fill="#1E3A8A" fontWeight="600">KNEE CLEARANCE</text>
          <text x="200" y="176" textAnchor="middle" fontFamily="Manrope, sans-serif" fontSize="6.5" fill="#1E3A8A">{d('27', '685')} high × {d('30', '760')} wide min</text>

          {/* Toe clearance zone */}
          <rect x="108" y="242" width="184" height="38" rx="2" fill="#7C3AED" opacity="0.04" stroke="#7C3AED" strokeWidth="1" strokeDasharray="5 3" />
          <text x="200" y="265" textAnchor="middle" fontFamily="Manrope, sans-serif" fontSize="6.5" fill="#5B21B6" fontWeight="600">TOE CLEARANCE (below {d('9', '230')})</text>
          ...
          <text x="520" y="180" fontFamily="Manrope, sans-serif" fontSize="6.5" fill="#1E3A8A" fontWeight="600">KNEE</text>

          {/* Toe clearance */}
          <rect x="468" y="248" width="140" height="32" rx="2" fill="#7C3AED" opacity="0.04" stroke="#7C3AED" strokeWidth="1" strokeDasharray="5 3" />
          <text x="538" y="268" textAnchor="middle" fontFamily="Manrope, sans-serif" fontSize="6.5" fill="#5B21B6" fontWeight="600">TOE ({d('9', '230')} high, {d('17–25', '430–635')} deep)</text>
      ...
      {ac && (
        <div ref={panelRef} style={{ marginTop: '12px', background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', animation: 'lavFade 0.25s ease-out' }}>
      ...
      <style>{`@keyframes lavFade { from { opacity:0; transform:translateY(-8px); } to { opacity:1; transform:translateY(0); } }        @media (prefers-reduced-motion: reduce) {
