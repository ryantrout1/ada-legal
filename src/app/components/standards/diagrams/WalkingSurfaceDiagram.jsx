  ...
  return (
    <div ref={panelRef} style={{ marginTop: '12px', background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', animation: 'walkFade 0.25s ease-out' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '1px solid var(--border)', background: 'var(--page-bg-subtle)', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <button onClick={onClose} aria-label="Close panel" style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '8px', padding: '8px 16px', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontWeight: 600, color: 'var(--body)', minHeight: '44px' }}>Close {'\u2715'}</button>
      </div>
...
function Dots({ callouts, active, toggle }) {
  return callouts.map(c => (
    <g key={c.id} tabIndex="0" role="button" aria-label={`Callout ${c.id}: ${c.label}`} aria-expanded={active === c.id} onClick={() => toggle(c.id)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggle(c.id); } }} style={{ cursor: 'pointer' }}>
      ...
      <circle cx={c.x} cy={c.y} r="22" fill="none" stroke="transparent" strokeWidth="2" className="walk-fr"/>
    </g>
  ));
}
  ...
  return (
    <div className="ada-diagram-wrap" style={{ margin: '32px 0' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', flexWrap: 'wrap', gap: '8px' }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.15rem', fontWeight: 700, color: 'var(--heading)', margin: 0 }}>Width & Slope</h2>
        {unitToggle}
      </div>
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
        <svg viewBox="0 0 720 360" role="group" aria-labelledby="ws-title" style={{ width: '100%', height: 'auto', display: 'block' }}>
          <title id="ws-title">Walking Surface Width and Slope Requirements</title>
          <rect width="720" height="360" fill="var(--page-bg-subtle)" />

          {/* LEFT: Width */}
          <text x="170" y="30" textAnchor="middle" fontFamily="var(--font-body)" fontSize="12" fontWeight="700" fill="var(--body-secondary)">How wide the path must be</text>

          {/* Corridor walls */}
          <rect x="60" y="80" width="220" height="14" rx="2" fill="var(--dx-line)" />
          <rect x="60" y="280" width="220" height="14" rx="2" fill="var(--dx-line)" />
          <text x="170" y="78" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-label)" fontWeight="600">wall</text>
          <text x="170" y="308" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-label)" fontWeight="600">wall</text>

          {/* Walking path */}
          <rect x="60" y="94" width="220" height="186" rx="0" fill="var(--dx-orange)" opacity="0.03" />

          {/* Wheelchair top-down */}
          <g transform="translate(135,145) scale(0.9)" opacity="0.5">
            <rect x="0" y="0" width="50" height="40" rx="4" fill="none" stroke="var(--dx-line)" strokeWidth="1.8" />
            <circle cx="25" cy="20" r="8" fill="var(--dx-line)" stroke="var(--dx-line)" strokeWidth="1.5" />
            <circle cx="5" cy="48" r="13" fill="none" stroke="var(--dx-line)" strokeWidth="1.5" />
            <circle cx="45" cy="48" r="13" fill="none" stroke="var(--dx-line)" strokeWidth="1.5" />
          </g>

          {/* 36" dimension */}
          <line x1="60" y1="320" x2="280" y2="320" stroke="var(--dx-orange)" strokeWidth="1.5" />
          <line x1="60" y1="314" x2="60" y2="326" stroke="var(--dx-orange)" strokeWidth="1.5" />
          <line x1="280" y1="314" x2="280" y2="326" stroke="var(--dx-orange)" strokeWidth="1.5" />
          <rect x="130" y="326" width="80" height="22" rx="6" fill="var(--dx-orange)" />
          <text x="170" y="341" textAnchor="middle" fontFamily="var(--font-body)" fontSize="11" fontWeight="700" fill="var(--page-bg)">{d('36', '915')} min</text>

          {/* DIVIDER */}
          <line x1="345" y1="20" x2="345" y2="340" stroke="var(--dx-line)" strokeWidth="1.5" strokeDasharray="6 4" />

          {/* RIGHT: Slope */}
          <text x="530" y="30" textAnchor="middle" fontFamily="var(--font-body)" fontSize="12" fontWeight="700" fill="var(--body-secondary)">How steep it can be</text>

          {/* Running slope surface */}
          <line x1="380" y1="230" x2="680" y2="190" stroke="var(--dx-line)" strokeWidth="3" />
          <text x="530" y="178" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-label)" fontWeight="600">direction of travel {'\u2192'}</text>

          {/* Running slope label */}
          <rect x="460" y="235" width="140" height="22" rx="6" fill="var(--dx-green)" />
          <text x="530" y="250" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fontWeight="700" fill="var(--page-bg)">running slope: 1:20 max</text>

          {/* Cross slope illustration */}
          <line x1="410" y1="300" x2="650" y2="295" stroke="var(--dx-line)" strokeWidth="2" strokeDasharray="4 3" />
          <rect x="460" y="305" width="140" height="22" rx="6" fill="var(--dx-blue)" />
          <text x="530" y="320" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fontWeight="700" fill="var(--page-bg)">cross slope: 1:48 max</text>

          {/* Slope angle indicator */}
          <line x1="380" y1="230" x2="680" y2="230" stroke="var(--dx-green)" strokeWidth="1" strokeDasharray="3 3" opacity="0.4" />
          <text x="690" y="212" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-green)" fontWeight="600">5%</text>

          {/* Warning note */}
          <rect x="395" y="90" width="270" height="44" rx="8" fill="var(--dx-orange)" opacity="0.05" stroke="var(--dx-orange)" strokeWidth="1" />
          <text x="530" y="110" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-orange)" fontWeight="600">Steeper than 1:20?</text>
          <text x="530" y="126" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-orange)">It{'\u2019'}s a ramp {'\u2014'} needs handrails + landings</text>

          <Dots callouts={SLOPE_CALLOUTS} active={slopeActive} toggle={toggleSlope} />
          <text x="20" y="350" fontFamily="var(--font-body)" fontSize="10" fill="var(--body-secondary)">Click or tap numbered callouts for details</text>
        </svg>
      </div>
      ...
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px 24px', marginTop: '12px' }}>
        <p style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', fontWeight: 700, color: 'var(--heading)', margin: '0 0 12px' }}>Key numbers {'\u2014'} Width & Slope</p>
        <KeyFact color="var(--dx-orange)" number={d('36', '915')}>Minimum clear width of any walking surface</KeyFact>
        <KeyFact color="var(--dx-green)" number="1:20">Maximum running slope (5%) before it becomes a ramp</KeyFact>
        <KeyFact color="var(--dx-blue)" number="1:48">Maximum cross slope (about 2%) to prevent wheelchair drift</KeyFact>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '40px', marginBottom: '8px', flexWrap: 'wrap', gap: '8px' }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.15rem', fontWeight: 700, color: 'var(--heading)', margin: 0 }}>Surface & Level Changes</h2>
      </div>
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
        <svg viewBox="0 0 720 320" role="group" aria-labelledby="surf-title" style={{ width: '100%', height: 'auto', display: 'block' }}>
          <title id="surf-title">Surface Requirements and Changes in Level</title>
          <rect width="720" height="320" fill="var(--page-bg-subtle)" />

          {/* LEFT: Surface */}
          <text x="170" y="30" textAnchor="middle" fontFamily="var(--font-body)" fontSize="12" fontWeight="700" fill="var(--body-secondary)">The surface itself</text>

          <rect x="40" y="70" width="280" height="50" rx="8" fill="var(--dx-violet)" opacity="0.04" stroke="var(--dx-violet)" strokeWidth="1.5" />
          <text x="180" y="92" textAnchor="middle" fontFamily="var(--font-body)" fontSize="11" fill="var(--dx-violet)" fontWeight="600">Firm + Stable + Slip-resistant</text>
          <text x="180" y="108" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-violet)">no loose gravel, deep carpet, or slick tile</text>

          <rect x="40" y="135" width="280" height="50" rx="8" fill="var(--dx-violet)" opacity="0.04" stroke="var(--dx-violet)" strokeWidth="1.5" />
          <text x="180" y="157" textAnchor="middle" fontFamily="var(--font-body)" fontSize="11" fill="var(--dx-violet)" fontWeight="600">Carpet: {d('\u00bd', '13')} max pile height</text>
          <text x="180" y="173" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-violet)">must be securely attached, firm backing</text>

          <rect x="40" y="200" width="280" height="50" rx="8" fill="var(--dx-violet)" opacity="0.04" stroke="var(--dx-violet)" strokeWidth="1.5" />
          <text x="180" y="222" textAnchor="middle" fontFamily="var(--font-body)" fontSize="11" fill="var(--dx-violet)" fontWeight="600">Grate openings: {d('\u00bd', '13')} max gap</text>
          <text x="180" y="238" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-violet)">long slots run perpendicular to travel</text>

          {/* DIVIDER */}
          <line x1="360" y1="20" x2="360" y2="300" stroke="var(--dx-line)" strokeWidth="1.5" strokeDasharray="6 4" />

          {/* RIGHT: Level changes */}
          <text x="540" y="30" textAnchor="middle" fontFamily="var(--font-body)" fontSize="12" fontWeight="700" fill="var(--body-secondary)">Bumps and edges</text>

          {/* Three tiers of level changes */}
          <rect x="400" y="70" width="280" height="55" rx="8" fill="var(--dx-green)" opacity="0.04" stroke="var(--dx-green)" strokeWidth="1.5" />
          <text x="540" y="90" textAnchor="middle" fontFamily="var(--font-body)" fontSize="11" fill="var(--dx-green)" fontWeight="600">{'\u2264'} {d('\u00bc', '6')} {'\u2014'} OK as-is</text>
          <text x="540" y="108" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-green)">vertical edge is fine</text>

          <rect x="400" y="140" width="280" height="55" rx="8" fill="var(--dx-amber)" opacity="0.04" stroke="var(--dx-amber)" strokeWidth="1.5" />
          <text x="540" y="160" textAnchor="middle" fontFamily="var(--font-body)" fontSize="11" fill="var(--dx-amber)" fontWeight="600">{d('\u00bc', '6')} to {d('\u00bd', '13')} {'\u2014'} must bevel</text>
          <text x="540" y="178" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-amber)">slope the edge at 1:2 max</text>

          <rect x="400" y="210" width="280" height="55" rx="8" fill="var(--dx-orange)" opacity="0.04" stroke="var(--dx-orange)" strokeWidth="1.5" />
          <text x="540" y="230" textAnchor="middle" fontFamily="var(--font-body)" fontSize="11" fill="var(--dx-orange)" fontWeight="600">{'\u003e'} {d('\u00bd', '13')} {'\u2014'} needs a ramp</text>
          <text x="540" y="248" textAnchor="middle" fontFamily="var(--font-body)" fontSize="10" fill="var(--dx-orange)">full ramp requirements apply ({'\u00a7'}405)</text>
      ...
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px 24px', marginTop: '12px' }}>
        <p style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', fontWeight: 700, color: 'var(--heading)', margin: '0 0 12px' }}>Key numbers {'\u2014'} Surface & Level Changes</p>
        <KeyFact color="var(--dx-violet)" number="3 rules">Firm, stable, and slip-resistant {'\u2014'} every walking surface</KeyFact>
        <KeyFact color="var(--dx-green)" number={`\u2264 ${d('\u00bc', '6')}`}>Small level changes OK as vertical edges</KeyFact>
        <KeyFact color="var(--dx-amber)" number={`${d('\u00bc', '6')}\u2013${d('\u00bd', '13')}`}>Must be beveled (angled at 1:2 max slope)</KeyFact>
        <KeyFact color="var(--dx-orange)" number={`> ${d('\u00bd', '13')}`}>Needs a full ramp with handrails and landings</KeyFact>
      </div>
