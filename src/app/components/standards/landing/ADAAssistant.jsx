/**
 * ADAAssistant — the Standards Guide hero search, ported from Base44
 * (src/components/guide/ADAAssistant.jsx @ 6b1e9ac) for M2 Phase 3.
 *
 * Despite sharing the hero with the AI helper, this component makes no
 * network call of any kind: it is a client-side keyword search over a
 * 76-entry index of chapters, guides and diagrams, with a combobox
 * results list. It was wrongly deferred to Phase 4 on the assumption
 * that it shared the helper's backend; it does not.
 *
 * Port seams:
 *   - Base44 flat routing (createPageUrl) → b44PageToRoute
 *   - useUniversalCta dropped. The flag only remapped the single
 *     "pathway" entry, and both CTA flags migrated to Neon as false in
 *     M0. The flag-off branch is taken, and the pathway entry points at
 *     /chat — RightsPathway was never built here, and Ada does that job.
 *   - trackEvent removed (analytics is M5).
 */
...
const CONTENT = [
  ...
  // Know Your Rights
  { id: "pathway", title: "Know Your Rights — Personalized Assessment", page: "RightsPathway", desc: "Answer a few questions about what happened and get a personalized summary of your legal rights, deadlines, remedies, and recommended next steps", keywords: "know rights personalized assessment quiz questionnaire what happened help which law applies deadline", category: "Know Your Rights" },
  ...
  { id: "guide-why-attorney", title: "Why You Need an ADA Attorney", page: "GuideWhyAttorney", desc: "How ADA enforcement works through private lawsuits, what attorneys do that you can't do alone, risks of filing pro se, and how ADA Legal Link connects you", keywords: "attorney lawyer why need ADA enforcement private lawsuit fee shifting contingency pro se risk AI filing connect help cost free", category: "Know Your Rights" },
...
function searchContent(query, content = CONTENT) {
...
export default function ADAAssistant() {
  ...
  // Both CTA flags migrated to Neon as false in M0, and the flag only
  // remapped the "pathway" entry. Flag-off branch: the index is used as
  // authored, and b44PageToRoute sends RightsPathway to /chat.
  const content = CONTENT;

  // Drop any entry whose page has no route here rather than rendering a
  // dead result. All 53 targets resolve today; this keeps that true if the
  // index or the route table drifts.
  const routable = useMemo(() => content.filter((item) => b44PageToRoute(item.page)), [content]);
  const results = useMemo(() => searchContent(query, routable), [query, routable]);
        ...
        >
          ...
          {results.map((item, idx) => {
            ...
            return (
              <Link
                key={item.id}
                to={b44PageToRoute(item.page) ?? '/standards-guide'}
                role="option"
