/**
 * CommunityVoices — sentiment poll, ported from Base44
 * (src/components/landing/CommunityVoices.jsx @ 6b1e9ac).
 *
 * Port seam: B44 read and wrote a CommunityVote entity that was never
 * actually created there, so every read fell into the catch and every
 * tally rendered zero. Its real votes went into the AnalyticsEvent
 * stream instead. Here both reads and writes go to /api/public/votes,
 * against a table seeded with those five historical votes — so this
 * widget shows true counts for the first time.
 */
  ...
  useEffect(() => {
    async function loadVotes() {
      try {
        ...
        const tallies = body?.tallies ?? {};
        setVotes({
          rights: tallies.rights ?? 0,
          happened: tallies.happened ?? 0,
          space: tallies.space ?? 0,
          believe: tallies.believe ?? 0,
        });
      } catch {
