    ...
    >
      <div style={{ maxWidth: '1160px', margin: '0 auto', padding: '0 2rem' }}>
        <div style={{ textAlign: 'center', maxWidth: '620px', margin: '0 auto 2.5rem' }}>
          ...
          <h2 id="v2-scope-heading" style={{ fontFamily: 'var(--font-body)', fontSize: '1.9rem', fontWeight: 800, color: 'var(--heading)', margin: '0 0 0.85rem', fontStyle: 'normal' }}>
            We'd rather tell you straight.
          </h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '1rem', color: 'var(--body)', lineHeight: 1.6, margin: 0 }}>
        ...
        <div className="v2-scope-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '22px', maxWidth: '880px', margin: '0 auto' }}>
          ...
          <div style={{ background: 'var(--page-bg)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px' }}>
            <h3 style={{ fontFamily: 'var(--font-body)', fontSize: '0.78rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--color-success-500)', margin: '0 0 18px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {CAN.map((item) => (
              <div key={item.title} style={{ marginBottom: '16px' }}>
                <b style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: '0.95rem', color: 'var(--heading)', fontWeight: 700, marginBottom: '3px' }}>{item.title}</b>
                <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.86rem', color: 'var(--body)', lineHeight: 1.55 }}>{item.desc}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: 'transparent', border: '1px dashed var(--border)', borderRadius: '16px', padding: '28px' }}>
            <h3 style={{ fontFamily: 'var(--font-body)', fontSize: '0.78rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--body-secondary)', margin: '0 0 18px' }}>
              Not yet
            </h3>
            {NOT.map((item) => (
              <div key={item.title} style={{ marginBottom: '16px' }}>
