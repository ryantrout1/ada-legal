...
describe('canAcceptSpotUpload', () => {
  it('caps the paid tier at 5 photos', () => {
    expect(MAX_PAID_PHOTOS).toBe(5);
  });
  ...
  it('allows uploads on a paid session under the cap', () => {
    ...
    expect(canAcceptSpotUpload('paid', 4)).toEqual({ ok: true });
  });

  it('refuses once the cap is reached', () => {
    expect(canAcceptSpotUpload('paid', 5)).toEqual({ ok: false, reason: 'limit_reached' });
    expect(canAcceptSpotUpload('paid', 6)).toEqual({ ok: false, reason: 'limit_reached' });
    // An in-flight session that already exceeded the old cap fails closed
    // rather than accepting more.
